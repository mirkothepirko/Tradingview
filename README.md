# TradingView MCP — Kell + Minervini VCP Edition

Fork of [LewisWJackson/tradingview-mcp-jackson](https://github.com/LewisWJackson/tradingview-mcp-jackson) — extended with an Oliver Kell Cycle of Price Action + Minervini VCP trading strategy, Pine Script indicator & strategy files, and backtest results across Mag7 stocks.

Built on top of the original [tradingview-mcp](https://github.com/tradesdontlie/tradingview-mcp) by [@tradesdontlie](https://github.com/tradesdontlie). Full credit to them for the foundation. [@LewisWJackson](https://github.com/LewisWJackson) added the morning brief workflow, rules config, and Desktop v2.14+ launch fix. This fork adds the Kell + Minervini VCP strategy layer on top.
...
|---------|-------------|
| **Kell + Minervini VCP Strategy** | Oliver Kell's 6-phase Cycle of Price Action combined with Minervini VCP — Pine Script indicator + strategy |
| **Pine Script files** | `scripts/kell_vcp_strategy.pine` (indicator) and `scripts/kell_vcp_strategy_backtest.pine` (strategy) |
| **Backtest results** | Mag7 backtests from 2022 with EMA50 exit — see `STRATEGY_INSIGHTS.md` |
| **Updated `rules.json`** | Full trading rules for the Kell + Minervini system |
| **German UI support** | `src/core/pine.js` patched to support German TradingView button labels |
...
Clone https://github.com/mirkothepirko/Tradingview.git to ~/tradingview-mcp-jackson, run npm install, then add it to my MCP config at ~/.claude/.mcp.json (merge with any existing servers, don't overwrite them). 
...
```bash
git clone https://github.com/mirkothepirko/Tradingview.git ~/tradingview-mcp-jackson
