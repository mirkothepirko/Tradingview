...
- **Automated daily scan (cross-platform):** `scripts/morning_scan.sh` (Linux/macOS, cron) and `scripts/morning_scan.ps1` (Windows, Task Scheduler) run the same pipeline: CDP-up check (self-heal + Telegram warn on failure) → **`scripts/wait_for_chart.js` warm-up** (polls `healthCheck()` until `api_available` + `chart_symbol` are set, default 60 s; Telegram warn on timeout — a partial briefing is **not** acceptable) → `watchlist sync` → `patterns -s watchlist` → `scripts/market_summary.js` + `scripts/scan_summary.js` (the shared Node formatters — keep both runners using them, no Python) → optional Telegram via `scripts/telegram_send.js` (token/chat-id from gitignored `.env`). Telegram delivery is **two separate HTML messages** (formatters and sender take `--html`): 1) Marktlage/Ampel + Wirtschaftskalender "Termine heute" (`scripts/econ_calendar.js --today` — free Forex-Factory JSON feed, USD/EUR, High+Medium; never fails hard, prints one notice line on feed errors), 2) Setups (HTF/Power Play). Plain text (no flag) remains the console/`.txt` archive format. Scheduling/autostart installers: `scripts/install_autostart_linux.sh`, `scripts/install_schedule_win.ps1`. Windows specifics in `WINDOWS_SETUP.md`.
- **Weekly watchlist review (Saturdays, cron):** `scripts/watchlist_review.js` aggregates the week's saved scan reports (`~/.tradingview-mcp/scans/YYYY-MM-DD.json` — fully offline, no TradingView needed) and sends a Telegram message sorting the watchlist into Streichkandidaten / Mittelfeld / verdient (thresholds are consts at the top of the file). It deliberately proposes **no new candidates** — the user adds those to the TradingView watchlist himself; the next morning scan picks them up via `watchlist sync`. Offline-tested in `tests/watchlist_review.test.js`.
